...
beddays_change <- case_when(
  beddays$perc_change > 1 ~ paste0("an increase of ", beddays$perc_change,
                                   "% compared"),
  beddays$perc_change < -1 ~ paste0("a decrease of ", abs(beddays$perc_change),
                                   "% compared"),
  TRUE ~ "similar"
)

and_but <- case_when(
  beddays$perc_change > 1 ~ "but",
  TRUE ~ "and"
)
...
* In `r month_name`, the average number of bed occupied per day due to delayed discharges was `r beddays$ave_bed_days`. This is `r beddays_change` to `r prev_month_name` when the daily average was `r prev_beddays`, `r and_but` lower than the monthly average prior to COVID-19 measures being put in place.
