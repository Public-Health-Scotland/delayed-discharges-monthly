...
# Version of R - 3.6.1
#
# Description - Creates Bed Days file by Council Area/Local Authority
#########################################################################
  ...
  mutate(
    ReasonForDelay = 
      case_when(
        ...
        str_detect(ReasonForDelay, 
                   "^Patient") ~ "Patient and Family Related Reasons",
        TRUE ~ ReasonForDelay
        )) %>%
...
beddays_ca %<>%
  mutate(AgeGroup = fct_relevel(as_factor(AgeGroup),
                                c("18plus", "18-74", "75plus")),
         ReasonForDelay = fct_relevel(as_factor(ReasonForDelay),
                                      c("All Delay Reasons",
                                        "Code 9 Reasons",
...
write_csv(
  ...
  here("output", year(pub_date(start_month)),
       ...
       glue("{format(start_month, '%Y-%m')}_",
            "delayed-discharge-beddays-council-area.csv"))
)
